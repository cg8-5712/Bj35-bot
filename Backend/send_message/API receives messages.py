# 对应接受消息回调模式中的EncodingAESKey 和 企业信息中的EncodingAESKey
...
# 对应步骤4中接受消息回调模式中的URL，如域名是'www.example.com'
# 那么在步骤4中填入的url就为"http://www.example.com/hook_path"
...
qy_api = [
    WXBizMsgCrypt(
        "Bj35",
        "7DYzzFxM4kIDoVYFTQL5n75LxNIvSgRBNALzWyxgaVF",
        "wwf7b517dadde6fcc6"),
]
    ...
    if (ret != 0):
        ...
        with open("/var/log/qywx.log", 'a+') as f:  # 消息接收日志
            ...
            if msg_type == "text":  # 文本消息
                ...
                f.write(
                    time.strftime('[%Y-%m-%d %H:%M:%S]') + "[ch%d] %s:%s\n" %
                    (i, name, msg))
                _thread.start_new_thread(os.system, (
                    # 此处将消息进行外部业务处理
                    "python3 command.py '%s' '%s' '%d' '%d'" % (name, msg, i, 0),))

            elif msg_type == "image":  # 图片消息
                name = name_xml[0].childNodes[0].data
                pic_url = pic_xml[0].childNodes[0].data
                f.write(
                    time.strftime('[%Y-%m-%d %H:%M:%S]') + "[ch%d] %s:图片消息\n" %
                    (i, name))
                _thread.start_new_thread(os.system, (
                    # 此处将消息进行外部业务处理
                    "python3 command.py '%s' '%s' '%d' '%d'" % (name, pic_url, i, 1),))
